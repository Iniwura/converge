...
async function loadBundle(objectiveId: string, address: string | null): Promise<Bundle> {
  ...
  const hasSubmitted = address ? Boolean(await readMethod("has_submitted", [objectiveId, address])) : null;
  return { objective, plans, synthesis, hasSubmitted };
}
...
function ObjectivePage({ objectiveId }: { objectiveId: string }) {
  ...
  const canSubmit = objective.state === "OPEN" && bundle.hasSubmitted !== true;
